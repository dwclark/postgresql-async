...
class TypeLoadTest extends Specification {
    ...
    def "Test Simple Automatic Serialization"() {
        ...
        def task = SimpleTask.forQuery(sql, [], { List<Object[]> list, Row row -> list << row.toArray(); }).toCompletable();
    ...
    def "Test All Types Automatic Serialization"() {
        ...
        def task = SimpleTask.forQuery(sql, [], { List<Map> list, Row row -> list << row.toMap(); }).toCompletable();
